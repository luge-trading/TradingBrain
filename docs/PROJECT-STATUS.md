...
TradingBrain 是个人 A 股与场内 ETF 的可解释量化研究、候选筛选、组合管理、条件式交易计划和盘后验证系统。当前已进入正式基线的链路覆盖行情获取、SQLite 存储、技术指标、风险分析、个股复盘、自选股批量复盘、每日汇总、交易日与收盘门控、macOS 自动调度和邮件通知；账户、组合、交易计划、模拟成交、回测及盘中系统仍是后续规划。
...
11. 个股对组合的收益贡献；
12. 是否跑赢基准；
13. 模型在回测、样本外验证和影子交易中是否有效。

核心输出不是一个精确价格预测，而是市场前提、候选标的、买入区间、触发条件、取消条件、仓位与数量、止损止盈、计划有效期和盘后执行验证。事实数据、派生指标、模型计划、模拟成交和真实人工成交必须保持明确分层。
...
- 声称能够预测日内最低点或最高点；
- 只展示成功计划而省略未触发、亏损和止损案例；
- 在没有时间戳和完整历史记录时评价模型准确性；
- 以命中价格区间代替净收益、风险和回撤评价。

所有输出仅用于研究和复盘，不构成投资建议。

## 3. Git 基线核验

当前真实状态必须以本地只读 Git 命令为准，本文不再把某一个 commit 永久声明为实时 `HEAD`。TASK-012D 的任务起始基线是 `020d7ab27e62e06e849cab0440d07a4c560e4e42`（`add industry sector data infrastructure`）；在 TASK-012D commit 和 push 闭环前，`HEAD` 与 `origin/main` 仍应保持该值。本文不虚构尚不存在的 TASK-012D commit SHA。
...
- `src/data/database.py`：股票、指数、市场和行业事实表的增量初始化、幂等保存和查询；批量读取当前行业注册表与行业日线面板，使用完整行业业务键关联，并支持 active/inactive、层级及日期过滤。
- `src/data/update.py`：股票、指数、市场及行业数据的相互独立更新协调。
- `src/analysis/technical.py`：基础技术指标计算。
- `src/analysis/sector_strength.py`：从行业与指数事实计算行业强弱快照；第一版固定使用 `SH000001` 统一交易观测日历和相对比较口径，计算行业收益、相对收益、成交活跃度、20 日高点距离和同层级排名。派生结果只在读取时计算，不持久化、不访问网络，也不生成交易建议。
...
- TASK-012C：东方财富三级行业注册表与不复权历史日线事实层已完成、正式提交并推送，commit 为 `020d7ab27e62e06e849cab0440d07a4c560e4e42`（`add industry sector data infrastructure`）；受控真实探针的环境阻塞及验收豁免记录继续保留。
- TASK-012D：行业相对强弱与排名的代码实现和全部离线验收已完成；正式 commit 和 push 状态以 Git 为准，不在闭环前记录未来 SHA。
...
公共接口为 `load_sector_daily_panel()`、`calculate_sector_strength_snapshot()` 和 `load_sector_strength_snapshot()`。`load_sector_daily_panel()` 使用完整行业业务键关联当前注册表与行业日线，支持 active/inactive、层级和日期过滤。固定基准 `DEFAULT_SECTOR_BENCHMARK_CODE = "SH000001"`；`SH000001` 只是第一版统一计算基准，无证据表明它完整代表全 A 市场，其主要作用是统一交易观测日历、起止日期和相对比较口径。
...
```text
return_Nd = 当前 close / 基准日历向前第 N 个交易观测日的行业 close - 1
relative_return_Nd = 行业同期收益 - SH000001 同期收益
amount_ratio_5d = 当前 amount / 前 5 个连续基准交易日 amount 均值
distance_to_20d_high = 当前 close / 最近 20 个对齐交易日最高 close - 1
```

N 日收益需要 N+1 个基准交易观测日期，行业必须拥有窗口内全部基准日期；不跳过缺失日期，不向更早记录补足，也不用上一交易日冒充 `as_of_date`。计算不使用未来数据：合法未来记录在事实校验前排除；非法日期无法确认是否为未来，仍然报错。缺失值保持缺失，不转换为 0。

一级、二级、三级分别按 `return_5d` 降序排名，并列使用 `method=min`。缺失 `return_5d` 不参与排名，`sector_count_5d` 只统计有效收益。默认只排名当前 active 行业，`active_only=False` 可显式包含 inactive 行业。当前 `is_active` 不是历史点时有效状态；数据库没有分类历史有效期，因此历史排名不具备无幸存者偏差保证。

TASK-012D 不新增行业强弱表，不保存排名、收益或相对收益；所有派生结果由事实数据实时重算。它未接入报告、邮件、engine 或 LaunchAgent，也不实现个股映射、候选评分或交易计划。

TASK-012D 验收事实：定向测试 `103 passed`；全量离线测试 `486 passed in 37.80s`；`pip check` 为 `No broken requirements found.`；静态审查为 `BLOCKER 0 / MAJOR 0 / MINOR 0`。临时 SQLite 已验证注册表、指数日线和行业日线写入读取，active/inactive、一级和二级排名隔离、收益、相对收益、`amount_ratio_5d`、`distance_to_20d_high` 及 nullable volume/amount/change_pct。

临时 SQLite 表集合仍只有 `stock_daily`、`index_daily`、`market_daily`、`sector_registry` 和 `sector_daily`，未创建派生强弱或排名表。正式数据库在探针前后的 size、mtime 和 SHA256 完全一致；验收未访问网络、正式复盘、邮件或 LaunchAgent。
...
当前下一步是完成 TASK-012D 的文档审查、再次全量验收、精确暂存、commit 和 push 闭环。

TASK-012D 闭环后的唯一下一任务是 TASK-012E：全市场个股横截面设计冻结。在此之前不得提前开始候选股评分、交易计划、分钟行情、盘中监控、回测或 Web 仪表盘。
