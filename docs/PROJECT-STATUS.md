...
- 分支：`main`
- Commit：`aeb67307f7f2d25d2e0397a789d6ded77f7238c7`
- Commit message：`add core index data infrastructure`
- 对应状态：TASK-012A 已完成、提交并推送。
...
- `src/data/database.py`：`stock_daily`、`index_daily`、`market_daily` 的增量初始化、幂等保存和查询。
- `src/data/update.py`：股票、指数和市场日度数据的相互独立更新协调。
...
- TASK-012B：市场基础环境数据层已实现并通过离线验收，待文档验收与受控暂存；尚未形成正式 commit。
...
- SQLite 数据域使用相互独立的 `stock_daily`、`index_daily` 和 `market_daily`；初始化只增量执行 `CREATE TABLE IF NOT EXISTS`，不得重建数据库或破坏已有表。
...
当前下一步是完成 TASK-012B 文档审查和精确受控暂存；在 TASK-012B commit/push 闭环前，不开始 TASK-012C。
